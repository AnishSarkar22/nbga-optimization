    ...
    for dataset, alt_dataset in zip(datasets, datasets_alt):
        ...
        try:
            if Path(dataset_path).exists():
                ...
                st.error(f"Dataset {dataset} not found in {dataset_dir}, skipping...")
                continue
        ...
        st.write(f"Testing on {dataset_name} ({len(distance_matrix)} cities)")
        
        for alg_name, alg_class in algorithms.items():
            with st.spinner(f"Running {alg_name}..."):
                ...
                if alg_name in ['OX_SIM', 'MOC_SIM']:
                    solver = alg_class(distance_matrix, pop_size=pop_size, 
                ...
                results[alg_name].append({
                    'distance': float(best_distance),  # Ensure serializable
                    'time': float(end_time - start_time),  # Ensure serializable
                    'tour': best_tour
                })
                
                st.write(f"{alg_name}: Distance = {best_distance:.2f}, Time = {end_time - start_time:.2f}s")
...
def main():
    st.set_page_config(
        page_title="TSP Algorithm Comparison",
        page_icon="🗺️",
        layout="wide"
    )
    
    st.title("🗺️ TSP Algorithm Comparison Tool")
    st.markdown("Comparing NBGA, SWAP_GATSP, OX_SIM, and MOC_SIM algorithms on TSP datasets")
    ...
    if st.sidebar.button("Run Comparison", type="primary"):
        ...
        st.info("⏳ Running algorithm comparison... This may take several minutes.")
        ...
        try:
            ...
            st.success(f"✅ Comparison completed in {total_time:.2f} seconds!")
            
            # Create and display the plot
            fig = create_comparison_plot(results, dataset_names, algorithm_names)
            st.pyplot(fig)
            ...
            for i, dataset_name in enumerate(dataset_names):
                ...
                cols = st.columns(len(dataset_results))
                for j, (alg, distance, exec_time) in enumerate(dataset_results):
                    with cols[j]:
                        rank_emoji = ["🥇", "🥈", "🥉", "4️⃣"][j] if j < 4 else f"{j+1}️⃣"
                        st.metric(
                            label=f"{rank_emoji} {alg}",
                            value=f"{distance:.2f}",
                            delta=f"{exec_time:.2f}s"
                        )
                
                st.write("---")
            
        except FileNotFoundError:
            st.error(f"❌ Dataset directory '{dataset_dir}' not found. Please check the path.")
        except Exception as e:
            st.error(f"❌ An error occurred: {str(e)}")
    
    # Information section
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📋 Algorithm Information")
    
    with st.sidebar.expander("NBGA", expanded=False):
        st.write("""
        **Neighbourhood-based Genetic Algorithm**
        - Uses neighbourhood-based crossover
        - Considers local distance penalties
        - Tournament selection
        - Neighbourhood-aware mutation
        """)
    
    with st.sidebar.expander("SWAP_GATSP", expanded=False):
        st.write("""
        **Genetic Algorithm with Swap Mutation**
    ...
    with st.sidebar.expander("OX_SIM", expanded=False):
        st.write("""
        **Order Crossover with Simulated Annealing**
        - Order crossover (OX)
    ...
    with st.sidebar.expander("MOC_SIM", expanded=False):
        st.write("""
        **Modified Order Crossover with Simulated Annealing**
        - Distance-aware crossover
        - Multiple segment selection
        - Enhanced SA with 2-opt and Or-opt
        - Tournament selection
        """)
    ...
    st.markdown("""
    1. **Prepare Dataset**: Place TSP (`.gz`/ `.tsp` files) files in the specified directory
    2. **Set Path**: Enter the correct dataset directory path
    3. **Run Comparison**: Click 'Run Comparison' to start the analysis
    4. **View Results**: Examine the comparison plot and performance tables
    
    The tool will automatically:
    - Parse TSP files (supports .gz compression)
    - Run all four algorithms with optimized parameters
    ...
    with st.expander("🔧 Technical Details", expanded=False):
        ...
        **Supported File Formats:**
        - .tsp and .tsp.gz files
        - TSPLIB standard format
        - EUC_2D and EXPLICIT edge weight types
        """)
